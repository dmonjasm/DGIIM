...
void receta::setNutricion(const ingredientes & all_ingre){
    float total_cal=0;
    float total_hc=0;
    float total_gras=0;
    float total_prot=0;
    ...
    ingredientes::const_iterator iter=all_ingre.begin();

    for(; it!=ings.end(); ++it){
        cantidad=((*it).second);

        for(iter=all_ingre.begin(); iter != all_ingre.end(); ++iter){
            if((*iter).getNombre() == (*it).first){
                total_cal+=((cantidad*(*iter).getCalorias())/100);
                total_hc+=((cantidad*(*iter).getHc())/100);
                total_gras+=((cantidad*(*iter).getGrasas())/100);
                total_prot+=((cantidad*(*iter).getProteinas())/100);
                total_fibra+=((cantidad*(*iter).getFibra())/100);
            }
        }  
    }
