...
const int num_items = 38 ,   // número de items
...
class Productor_consumidor : public HoareMonitor
{
   ...
   public:
      ...
      int sacar_de_buffer(){
         ...
         if(contador_lecturas% 5==0 || num_items-contador_total==0){
             cola_contadora.signal();
         }
      ...
      int esperar5extraidos(){
        cola_contadora.wait();
        cout << "Se han extraído " << contador_lecturas << " datos nuevos" << endl;
        cout << "El valor de su suma es: " << suma_valores << endl;
        suma_valores=0;
        contador_lecturas=0;

        return (num_items - contador_total);
...
void funcion_hebra_contadora(MRef<Productor_consumidor> monitor){
    int respuesta=1;

    while(respuesta != 0){
        respuesta=monitor->esperar5extraidos();
    }
}
//----------------------------------------------------------------------
