...
const int n_p = 5, n_c = 8;
int p = num_items/n_p, c = num_items / n_c;

int vector_1[n_p], vector_2[n_c];
...
{
   static int contador = num_hebra*p;
   this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));
   mtx.lock();
   cout << "Hebra " << num_hebra << " ha producido: " << contador << endl << flush ;
   cont_prod[contador] ++ ;
   vector_1[num_hebra]++;
   mtx.unlock();
   return contador++ ;
...
int main()
{
   ...
   thread vector_hebras_prodcutoras[n_p], vector_hebras_consumidoras[n_c];

   for(int i = 0; i < n_p; i++){
      vector_hebras_prodcutoras[i]=thread(funcion_hebra_productora, &monitor, i);
   }

   for(int i = 0; i < n_c; i++){
      vector_hebras_consumidoras[i]=thread(funcion_hebra_consumidora, &monitor,i);
   }

   for(int i = 0; i < n_p; i++){
      vector_hebras_prodcutoras[i].join();
   }

   for(int i = 0; i < n_c; i++){
      vector_hebras_consumidoras[i].join();
   }
