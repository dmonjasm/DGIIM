...
class ProdConsSU : public HoareMonitor
{
 ...
 int                        // variables permanentes
   ...
   primera_ocupada,
   num_celdas_ocupadas,
   multiplos_cinco_totales,
   multiplos_cinco_nuevos;
...
ProdConsSU::ProdConsSU(  )
{
   ...
   multiplos_cinco_totales=0;
   multiplos_cinco_nuevos=0;
}
// -----------------------------------------------------------------------------
...
bool ProdConsSU::metodo_impresora()
{
    if(num_items/5 == multiplos_cinco_totales){
        return false;
    }

    else{
        if(multiplos_cinco_nuevos==0){
            impresora.wait();
        }

        cout << "Se han insertado " << multiplos_cinco_nuevos << " en el buffer" << endl;
        multiplos_cinco_nuevos = 0;
...
void funcion_hebra_impresora( MRef<ProdConsSU> monitor)
{ 
    bool no_terminado=true;
    while(no_terminado){
        no_terminado=monitor->metodo_impresora();
    }
}
