...
void consumir_dato( unsigned dato, int hebra )
{
   ...
   this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));
   mtx.lock();
   cout << "\t\tHebra " << hebra << " consumido: " << dato << endl << flush;
   mtx.unlock();
}
//----------------------------------------------------------------------
...
class ProdCons1SC
{
 ...
 int                        // variables permanentes
   ...
   primera_ocupada, num_celdas_ocupadas;
...
void funcion_hebra_productora( ProdCons1SC * monitor, int num_productora )
{
...
void funcion_hebra_consumidora( ProdCons1SC * monitor, int num_consumidor)
{
