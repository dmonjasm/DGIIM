...
const int 
   ...
   etiq_test_prod=4,         //etiqueta de los mensajes del productor al test
   etiq_test_cons=3;         //etiqueta de los mensajes del consumidor al test
...
void funcion_test(){
   ...
   int test_productores[num_items]={0},  //Vectores tipo int que servirán para comprobar la corrección del programa         
       test_consumidores[num_items]={0};
   int valor;
   MPI_Status estado;
   bool ok=true;

   for(int i=0; i < num_items; i++){
      MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_test_prod, MPI_COMM_WORLD, &estado);

      test_productores[valor-1]++;

      MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_test_cons, MPI_COMM_WORLD, &estado);

      test_consumidores[valor-1]++;
   }

   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      if ( test_productores[i] != 1 )
      {
         cout << "error: valor " << i+1 << " producido " << test_productores[i] << " veces." << endl ;
         ok = false ;
      }

      else{
      cout << "Se ha producido el valor " << i+1 << " en total " << test_productores[i] << " veces" << endl;
      }
   ...
   for(unsigned i =0; i < num_items; i++)
   {
      if ( test_consumidores[i] != 1 )
      {
         cout << "error: valor " << i+1 << " consumido " << test_consumidores[i] << " veces" << endl ;
         ok = false ;
      }

      else{
         cout << "Se ha consumido el valor " << i+1 << " en total " << test_consumidores[i] << " veces" << endl;
      }
   }
   if (ok)
